...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        // TODO(proj4_part2): B+ tree locking
        LeafNode getNode = this.root.get(key);
        return new BPlusTreeIterator(getNode, getNode.scanGreaterEqual(key));
    ...
    public void put(DataBox key, RecordId rid) {
        typecheck(key);
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> retVal = this.root.put(key, rid);

        if (!retVal.isPresent()) {
            return;
        }

        //now we know that the root did split because it sent up a value.

        List<DataBox> newKeys = new ArrayList<>();
        newKeys.add(retVal.get().getFirst());
        List<Long> newChildren = new ArrayList<>();
        newChildren.add(this.root.getPage().getPageNum());
        newChildren.add(retVal.get().getSecond());
        ...
        updateRoot(new InnerNode(this.metadata, this.bufferManager, newKeys, newChildren, lockContext));
        // TODO(proj4_part2): B+ tree locking
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        // TODO(proj2): implement
        BPlusNode leftNode = this.root.getLeftmostLeaf();
        if (leftNode != this.root)
        {
            throw new BPlusTreeException("tree is not empty at time of bulk loading");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> retVal = this.root.bulkLoad(data, fillFactor);
            if (retVal.isPresent()) {
                //now we need to do similar to put where we know its split so new root
                List<DataBox> newKeys = new ArrayList<>();
                newKeys.add(retVal.get().getFirst());
                List<Long> newChildren = new ArrayList<>();
                newChildren.add(this.root.getPage().getPageNum());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private Iterator<RecordId> iterCurr;
        private LeafNode leaf;
        public BPlusTreeIterator(LeafNode leaf, Iterator<RecordId> iterCurr){
            this.leaf = leaf;
            this.iterCurr = iterCurr;

            //need to find first node that isnt empty, since we can remove
            if (!iterCurr.hasNext()) {
                goToNextNode();
            }
        }
        private void goToNextNode() {
            Optional<LeafNode> holdLeaf = leaf.getRightSibling();
            if (holdLeaf.isPresent()){
                this.leaf = holdLeaf.get();
                this.iterCurr = this.leaf.scanAll();

                //since there are no values on this leaf, we go to next
                if (!iterCurr.hasNext()) {
                    goToNextNode();
                }
            }
