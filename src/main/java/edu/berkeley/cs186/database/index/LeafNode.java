...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid)
    {
        ...
        // TODO(proj2): implement
            ...
            int indexToAddTo = InnerNode.numLessThanEqual(key, this.keys);
            for (DataBox keyInNode : this.keys) {
                if (key.equals(keyInNode)) {
                    String exceptionString = "Key already in page, excpetion thrown";
                    throw new BPlusTreeException(exceptionString);
                }
            }

        //if not full, add key and rid
        int d = this.metadata.getOrder();
        if (this.keys.size() <= 2 * d - 1) {
            this.keys.add(indexToAddTo, key);
            this.rids.add(indexToAddTo, rid);
            sync();
            return Optional.empty();
        }

        //if full, we need to split into 2 leaf nodes where one has d elements, other has d+1, and send up the first in d+1
        ...
        for (int i = d; i < 2 * d + 1; i++) {
            newKeys.add(this.keys.get(d));
            newRids.add(this.rids.get(d));
        }


        this.keys.subList(d, 2 * d + 1).clear();
        this.rids.subList(d, 2 * d + 1).clear();
  /*  LeafNode(BPlusTreeMetadata metadata, BufferManager bufferManager, List<DataBox> keys,
             List<RecordId> rids, Optional<Long> rightSibling, LockContext treeContext)  */
       /* System.out.print("In left node keys are :");
        for (int i = 0; i < this.keys.size(); i++) {
            System.out.print(" " + keys.get(i).toString());
        }
        System.out.println();

        System.out.print("In right node keys are :");
        for (int i = 0; i < newKeys.size(); i++) {
            System.out.print(" " + newKeys.get(i).toString());
        }
        System.out.println();*/

        LeafNode newNode = new LeafNode(this.metadata, this.bufferManager, newKeys, newRids, this.rightSibling, this.treeContext);
        long pageNumber = newNode.getPage().getPageNum();


        this.rightSibling = Optional.of(pageNumber);
        sync();
        return Optional.of(new Pair<>(newKeys.get(0), pageNumber));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        //Pair nextPair = data.next();

      /*  int indexToAddTo = InnerNode.numLessThanEqual((DataBox) nextPair.getFirst(), this.keys);
        for (DataBox keyInNode : this.keys) {
            if (nextPair.getFirst().equals(keyInNode)) {
                String exceptionString = "Key already in page, excpetion thrown";
                throw new BPlusTreeException(exceptionString);
            }
        }
       */
        ...
        int fillTo = (int) Math.ceil(2*d*fillFactor);
        // TODO(proj2): implement
        for (int i = this.keys.size(); i <= fillTo - 1 && data.hasNext(); i++) {
        ...
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();
        newKeys.add(nextPair.getFirst());
        newRids.add(nextPair.getSecond());
        LeafNode newNode = new LeafNode(this.metadata, this.bufferManager, newKeys, newRids, this.rightSibling, this.treeContext);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> b = Optional.of(rightSibling);
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, b, treeContext);
