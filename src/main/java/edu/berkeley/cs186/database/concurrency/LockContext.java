...
public class LockContext {
    ...
    throws InvalidLockException, DuplicateLockRequestException {
        ...
        if (this.readonly) {
            throw new UnsupportedOperationException("context " + this.toString() + " is read only");
        }
        if (this.lockman.getLockType(transaction, this.name) == lockType) {
            throw new DuplicateLockRequestException("lock is already held by " + transaction.toString());
        }
    ...
    throws NoLockHeldException, InvalidLockException {
        ...
        if (this.lockman.getLockType(transaction, this.name) == LockType.NL) {
            throw new NoLockHeldException("No lock held on " + this.name.toString() + " held by " + transaction.toString());
        }
        ...
        if (parent != null) {
            if (parent.numChildLocks.get(transaction.getTransNum()) != null) {
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        if (readonly) {
            throw new UnsupportedOperationException(this.toString() + " is readonly");
        }
        // TODO(proj4_part2): implement
        if (lockman.getLockType(transaction, name) == newLockType) {
            throw new DuplicateLockRequestException(transaction.toString() + " already has a " + newLockType.toString() + " lock");
        }
        if (lockman.getLockType(transaction, name) == LockType.NL) {
            throw new NoLockHeldException(transaction.toString() + " has no lock");
        }
        boolean invalidState = false;
        boolean promotionIsSIX = false;
        if (newLockType == LockType.S) {

        }
        if (parent != null) {
            if (newLockType == LockType.X) {
                if (lockman.getLockType(transaction, parent.name) == LockType.S || lockman.getLockType(transaction, parent.name) == LockType.IS) {
                    invalidState = true;
                }
            }
            if (newLockType == LockType.IX) {
                if (lockman.getLockType(transaction, parent.name) == LockType.S || lockman.getLockType(transaction, parent.name) == LockType.IS) {
                    invalidState = true;
                }
            }
        ...
        if (invalidState || !((LockType.substitutable(newLockType, lockman.getLockType(transaction, name)) && newLockType != lockman.getLockType(transaction, name))
                || (newLockType == LockType.SIX &&
                (lockman.getLockType(transaction, name) == LockType.IS || lockman.getLockType(transaction, name) == LockType.IX|| lockman.getLockType(transaction, name) == LockType.S)))) {
            throw new InvalidLockException("not a promotion or would cause invalid state");
        } else {
            if (!promotionIsSIX) {
            ...
            if (promotionIsSIX) {
                if (sisDesc.size() != 0) {
                    this.numChildLocks.put(transaction.getTransNum(), this.numChildLocks.get(transaction.getTransNum()) - sisDesc.size());
                }
                sisDesc.add(name);
                lockman.acquireAndRelease(transaction, name, LockType.SIX, sisDesc);
                //for (ResourceName a: sisDesc) {
                //    lockman.release(transaction, a);
                //}
            }
        }
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if (lockman.getLockType(transaction, name) == LockType.NL) {
            throw new NoLockHeldException(transaction.toString() + " has no lock at this level");
        }
        ...
        for (Lock a : lockList) {
            if (a.name.isDescendantOf(name) || a.name == name) {

                if (a.lockType == LockType.SIX || a.lockType == LockType.IX || a.lockType == LockType.X) {
                    escToS = false;
                }
            }
        }
        ...
        for (Lock a : lockList) {
            if (a.name.isDescendantOf(name) || a.name == name) {
                resources.add(a.name);
            }
        }
        if (escToS) {
            if (numChildLocks.get(transaction.getTransNum()) != null) {
                this.numChildLocks.put(transaction.getTransNum(), this.numChildLocks.get(transaction.getTransNum()) - (resources.size() - 1));
            }

            if (lockman.getLockType(transaction, name) != LockType.S) {
                lockman.acquireAndRelease(transaction, name, LockType.S, resources);
            }
        }
        else {
            if (numChildLocks.get(transaction.getTransNum()) != null) {
            ...
            if (lockman.getLockType(transaction, name) != LockType.X) {
                lockman.acquireAndRelease(transaction, name, LockType.X, resources);
            }
        }
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        LockContext up = parent;
        while (up != null) {

            if (lockman.getLockType(transaction, up.name) == LockType.X) {
                return LockType.X;
            }
            if (lockman.getLockType(transaction, up.name) == LockType.SIX) {
                return LockType.S;
            }
            if (lockman.getLockType(transaction, up.name) == LockType.S) {
                return LockType.S;
            }
            up = up.parent;
        }
    ...
    private boolean hasSIXAncestor(TransactionContext transaction) {
        // TODO(proj4_part2): implement
        for (Lock a: lockman.getLocks(transaction)){
            if (!a.name.isDescendantOf(name) && (a.lockType == LockType.SIX) && (a.name != name)) {
                return true;
            }
        }
        return false;
    ...
    private List<ResourceName> sisDescendants(TransactionContext transaction) {
        ...
        for (Lock a: lockman.getLocks(transaction)){
            if (a.name.isDescendantOf(name) && (a.lockType == LockType.S || a.lockType == LockType.IS)) {
                lockList.add(a.name);
            }
        }

        //If you iterate through all the current locks for a transaction using LockManager#getLocks, and collect the locks that satisfy ResourceName#isDescendantOf then you should get the list of all descendants.
        return lockList;
    }
