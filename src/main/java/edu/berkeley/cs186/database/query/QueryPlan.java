...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Map<String, QueryOperator>  firstPassMap = new HashMap<>();
        for (String name: this.joinTableNames) {
            firstPassMap.put(name, minCostSingleAccess(name));
        }
        firstPassMap.put(this.startTableName, minCostSingleAccess(this.startTableName));
        ...
        // been joined.
        Map<Set, QueryOperator>  passI = new HashMap<>();
        for (String a: firstPassMap.keySet()) {
            Set<String> newSet = new HashSet<>();
            newSet.add(a);
            passI.put(newSet, firstPassMap.get(a));
        }
        Map<Set, QueryOperator> firstCopy = new HashMap<>(passI);

        for (int i = 0; i < joinTableNames.size(); i++) {
            passI = minCostJoins(passI, firstCopy);
        }
        ...
        this.finalOperator = minCostOperator(passI);
        addGroupBy();
        addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int count = 0;
        for (Integer holdd : list) {
            //System.out.println(columnName.substring(table.length() + 1));
            IndexScanOperator indexScan = new IndexScanOperator(this.transaction, table, selectColumnNames.get(holdd), this.selectOperators.get(list.get(count)), this.selectDataBoxes.get((list.get(count))));


            int costIndex = indexScan.estimateIOCost();
            if (costIndex < lowestCost) {
                numOfIndex = list.get(count);
                minOp = indexScan;
                lowestCost = costIndex;
            }
            count++;
        }
        ...
        minOp = addEligibleSelections(minOp, numOfIndex);
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Set key: prevMap.keySet()){
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                String leftColumn = this.getJoinLeftColumnNameByIndex(i)[1];
                String leftTable = this.getJoinLeftColumnNameByIndex(i)[0];

                String rightColumn = this.getJoinRightColumnNameByIndex(i)[1];
                String rightTable = this.getJoinRightColumnNameByIndex(i)[0];

                QueryOperator newQueryOp = null;
                Set newSet = new HashSet<>(key);

                //case 1
                if (key.contains(leftTable) && !key.contains(rightTable)) {
                    Set<String> set = new HashSet<>();
                    set.add(rightTable);
                    newQueryOp = minCostJoinType(prevMap.get(key), pass1Map.get(set), leftColumn, rightColumn);
                    newSet.add(rightTable);
                    }

                    //currentCost = Integer.min(currentCost, minCostJoinType(prevMap.get(key), pass1Map.get(rightTable), leftColumn, rightColumn).getIOCost());
                ...
                //case 2
                if (key.contains(rightTable) && !key.contains(leftTable)) {
                    Set<String> set = new HashSet<>();
                    set.add(leftTable);
                    newQueryOp = minCostJoinType(pass1Map.get(set), prevMap.get(key), leftColumn, rightColumn);
                    newSet.add(leftTable);
                   /* if (currentCost > newQueryOp.getIOCost()) {
                        currentCost = newQueryOp.getIOCost();
                        opToMapTo = newQueryOp;
                    }*/
                }
                //case 3
                if ((key.contains(rightTable) && key.contains(leftTable)) || (!key.contains(rightTable) && !key.contains(leftTable))) {
                    continue;
                }
                if (!map.containsKey(newSet) || map.get(newSet).getIOCost() > newQueryOp.getIOCost()) {
                    map.put(newSet, newQueryOp);
                }
            }
