...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        // Our hash table to build on. The list contains all the records in the --- records that hash to the same key
        ...
        while (probeRecords.hasNext()) {
            ...
            if (hashTable.containsKey(probeJoinValue)) {
                ...
                for (Record newRecord : hashTable.get(probeJoinValue)) {
                    //   boolean probeFirst; // True if the probe records come from the left partition
                    Record joinedRecord;
                    if (probeFirst){
                        joinedRecord = joinRecords(probeRecord, newRecord);
                    }
                    else {
                        joinedRecord = joinRecords(newRecord, probeRecord);
